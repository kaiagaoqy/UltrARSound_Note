...
// Using #IF will determine if the code is compiled or not. (only for compiler constants that won't exist in runtime)
...
// use research mode
    ...
    void Start()
    {
        ...
        // Initialize simulated spheres
        KFspheres = new KalmanFilterSphere[4];//Defined 4 spheres
        ...
        for (int i = 0; i < KFspheres.Length; i++)
        {
            GameObject sphere = GameObject.CreatePrimitive(PrimitiveType.Sphere);// Creates a game object with a primitive mesh renderer and appropriate collider. --> Create a simulated sphere in unity
            sphere.tag = "sphere";
...
#endif
        // Initilize blob detection algrm to detect spheres in the AB image
        ...
        // Load LookUp table L = 512 x 512 x 3
        // Here assumed that the static lookup table is perfect
        var loadedLUT = Resources.Load("AHaT_lut") as TextAsset; // declare local variables without giving them explicit types. The Resources class allows you to find and access Objects including assets.
        bytesLUT = loadedLUT.bytes;
        LUT = new float[512 * 512 * 3]; //3 channels
        ...
        Core.rotate(matLUTimage, matLUTimageRot, 0); // rotate (Mat inputarr, Mat outputarr, int rotateCode) --> Rotate img by 90 degrees clockwise 
    ...
    #region Setup 4 simulated spheres in Unity
...
// Polaris Stylus
    ...
    // Calculate relative distance to the first sphere
    ...
    #region Compute Initial Guess of D_s = d_p.L(p)
    // Compute Initial Guess of surface position of spheres in the depth camera coordinate
    void calculatePosSphere(ushort[] depthMapArray, List<float> listBlobsInImage)
    {
        posSpheresInCamSpaces = new List<Vector3>();
        var depthMapArrayShort = Array.ConvertAll(depthMapArray, item => (short)item); // => Lambda operator, pass item in, return (short)item
        Mat matDepthImage = new Mat(512, 512, CvType.CV_16U); //CV_16U = 2
        matDepthImage.put(0, 0, depthMapArrayShort);
        Core.rotate(matDepthImage, matDepthImage, 0); // 2*512*512

        for (int i = 0; i < listBlobsInImage.Count; i = i + 2)
        {
            int x_pixel = Convert.ToInt32(Math.Round(listBlobsInImage[i])); // pixel coordinates p_i = (x_pixel, y_pixel)
            int y_pixel = Convert.ToInt32(Math.Round(listBlobsInImage[i + 1]));
            var depthMapScalar2 = matDepthImage.get(x_pixel, y_pixel); // get corresponding pixel value d_p in the depth image
            var uvw = matLUTimageRot.get(x_pixel, y_pixel);  // Entry of the LUT at image position p
    ...
    List<float> getBlobsInImage(Mat imgMat)
    {
        ...
        var blobArray = blobs.toArray(); // return KeyPoint[]
    ...
    #region Declare a Coroutine to track blobs
    ...
    {
        #region Pose of the left front cam T^w_L 
#if ENABLE_WINMD_SUPPORT
        ...
        Matrix4x4 world2DepthCam = Matrix4x4.TRS(posRigidNode, rotRigidNode, new Vector3(1, 1, 1)); //Creates a translation, rotation and scaling matrix.
    ...
    public void StartTrackingCoroutine()
    {
        ...
        //In Unity, a coroutine is a method that can pause execution and return control to Unity
...
public static class MatrixExtensions
{
    public static Quaternion ExtractRotation(this Matrix4x4 matrix) 
        // Unity uses the Quaternion class to store the three dimensional orientation of GameObjects
        // as well as using them to describe a relative rotation from one orientation to another.
    {
        ...
        return Quaternion.LookRotation(forward, upwards); // Quaternion rotation
    }
    ...
    {
        Vector3 scale;
        scale.x = new Vector4(matrix.m00, matrix.m10, matrix.m20, matrix.m30).magnitude; //square root of (x*x+y*y+z*z+w*w).
